"""autocreator.py — Build Wallpaper Engine projects from videos alone.

Unlike creator.py (which needs a matching preview file to already exist), this
engine *generates* the preview straight from the video with ffmpeg.

For every video in the source folder it:
  1. Creates  target/<name>-<random_suffix>   (same naming logic as creator.py)
...
Folder naming and project.json are reused verbatim from creator.py, so projects
made here are indistinguishable from the ones the Creator tab produces.
...
from .creator import build_project_json, generate_suffix
